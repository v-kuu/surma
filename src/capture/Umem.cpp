...
std::expected<Umem, UmemError> Umem::init(Platform &platform)
{
	...
	int res = platform.xsk_umem__create(
	    &ret.umem_, ret.area_, UMEM_SIZE, &ret.fq_, &ret.cq_, &cfg);

	if (res != 0)
	{
		spdlog::error(
		    "xsk_umem__create failed: ret={}, errno={}, {}",
		    res,
		    errno,
		    std::strerror(errno));
...
std::expected<void, UmemError> Umem::populate_fill_queue_()
{
	...
	if (n < FILL_RING_SIZE)
		spdlog::warn(
		    "fill queue reserve returned {} of {} slots", n, FILL_RING_SIZE);
