...
std::expected<XdpProgram, XdpError> XdpProgram::load(
    Platform &platform,
    int ifindex)
{
	XdpProgram ret{};
	...
	{
		spdlog::error(
		    "failed to load xdp program: {} ({})", -res, std::strerror(-res));
		return std::unexpected(XdpError::LoadError);
	}
	return ret;
}

std::expected<void, XdpError> XdpProgram::attach(
    Platform &platform,
    Socket &socket) const
{
	...
	{
		spdlog::error(
		    "failed to attach xdp program to socket: {} ({})",
		    -res,
		    std::strerror(-res));
