...
struct Platform
{
	...
	virtual uint32_t xsk_ring_prod__reserve(
	    struct xsk_ring_prod *fq,
	    int size,
	    uint32_t *idx) = 0;
	virtual unsigned long long *xsk_ring_prod__fill_addr(
	    struct xsk_ring_prod *fq,
	    uint32_t idx) = 0;
	virtual void xsk_ring_prod__submit(
	    struct xsk_ring_prod *fq,
	    uint32_t n) = 0;

	virtual void xsk_socket__delete(struct xsk_socket *xsk) = 0;
	virtual int xsk_socket__create(
...
struct LinuxPlatform : Platform
{
	...
	uint32_t xsk_ring_prod__reserve(
	    struct xsk_ring_prod *fq,
	    int size,
	...
	unsigned long long *xsk_ring_prod__fill_addr(
	    struct xsk_ring_prod *fq,
	    uint32_t idx) override
	{
